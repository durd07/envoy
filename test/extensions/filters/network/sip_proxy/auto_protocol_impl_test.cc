...
#include "extensions/filters/network/sip_proxy/auto_protocol_impl.h"
#include "extensions/filters/network/sip_proxy/binary_protocol_impl.h"
#include "extensions/filters/network/sip_proxy/compact_protocol_impl.h"
#include "extensions/filters/network/sip_proxy/twitter_protocol_impl.h"
...
TEST_F(AutoProtocolTest, UnknownProtocol) {
  ...
  EXPECT_THROW_WITH_MESSAGE(proto.readMessageBegin(buffer, metadata_), EnvoyException,
                            "unknown sip auto protocol message start 0102");
  expectDefaultMetadata();
}
...
TEST_F(AutoProtocolTest, ProtocolUpgradeDelegation) {
  ...
  // attemptUpgrade
  {
    ...
    Buffer::OwnedImpl buffer;
    SipObject* obj = new NiceMock<MockSipObject>();
