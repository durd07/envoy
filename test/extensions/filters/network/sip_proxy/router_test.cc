...
public:
  ...
  void startRequestWithExistingConnection(MessageType msg_type) {
    ...
    if (!conn_state_) {
      conn_state_ = std::make_unique<SipConnectionState>();
    }
    EXPECT_CALL(*context_.cluster_manager_.thread_local_cluster_.tcp_conn_pool_.connection_data_,
                connectionState())
        .WillRepeatedly(
  ...
  void returnResponse(MessageType msg_type = MessageType::Reply, bool is_success = true) {
    ...
    EXPECT_CALL(callbacks_, upstreamData(Ref(buffer)))
        .WillOnce(Return(SipFilters::ResponseStatus::MoreData));
    upstream_callbacks_->onUpstreamData(buffer, false);

    EXPECT_CALL(callbacks_, upstreamData(Ref(buffer)))
        .WillOnce(Return(SipFilters::ResponseStatus::Complete));
    EXPECT_CALL(context_.cluster_manager_.thread_local_cluster_.tcp_conn_pool_,
                released(Ref(upstream_connection_)));
    upstream_callbacks_->onUpstreamData(buffer, false);
  }
...
class SipRouterFieldTypeTest : public testing::TestWithParam<FieldType>, public SipRouterTestBase {
public:
};

INSTANTIATE_TEST_SUITE_P(PrimitiveFieldTypes, SipRouterFieldTypeTest,
...
class SipRouterContainerTest : public testing::TestWithParam<FieldType>, public SipRouterTestBase {
public:
};

INSTANTIATE_TEST_SUITE_P(ContainerFieldTypes, SipRouterContainerTest,
                         Values(FieldType::Map, FieldType::List, FieldType::Set),
                         fieldTypeParamToString);

class SipRouterPassthroughTest
...
TEST_F(SipRouterTest, PoolRemoteConnectionFailure) {
  ...
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_rq_call")
                     .value());
...
TEST_F(SipRouterTest, PoolLocalConnectionFailure) {
  initializeRouter();
...
TEST_F(SipRouterTest, PoolTimeout) {
  initializeRouter();
...
TEST_F(SipRouterTest, PoolOverflowFailure) {
  initializeRouter();
...
TEST_F(SipRouterTest, PoolConnectionFailureWithOnewayMessage) {
  initializeRouter();
  startRequest(MessageType::Oneway);

  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_rq_oneway")
                     .value());
...
TEST_F(SipRouterTest, NoCluster) {
  initializeRouter();
  initializeMetadata(MessageType::Call);
...
TEST_F(SipRouterTest, TruncatedResponse) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
...
TEST_F(SipRouterTest, UpstreamRemoteCloseMidResponse) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
...
TEST_F(SipRouterTest, UpstreamLocalCloseMidResponse) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
...
TEST_F(SipRouterTest, UpstreamCloseAfterResponse) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  sendTrivialStruct(FieldType::String);
...
TEST_F(SipRouterTest, UpstreamDataTriggersReset) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  sendTrivialStruct(FieldType::String);
  ...
  EXPECT_CALL(callbacks_, startUpstreamResponse(_, _));
  EXPECT_CALL(callbacks_, upstreamData(Ref(buffer)))
      .WillOnce(Return(SipFilters::ResponseStatus::Reset));
...
TEST_F(SipRouterTest, UnexpectedUpstreamRemoteClose) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  sendTrivialStruct(FieldType::String);
...
TEST_F(SipRouterTest, UnexpectedUpstreamLocalClose) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  sendTrivialStruct(FieldType::String);
...
TEST_F(SipRouterTest, DontCloseConnectionTwice) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  sendTrivialStruct(FieldType::String);
...
TEST_F(SipRouterTest, UnexpectedRouterDestroyBeforeUpstreamConnect) {
  initializeRouter();
  startRequest(MessageType::Call);
...
TEST_F(SipRouterTest, UnexpectedRouterDestroy) {
  initializeRouter();
  startRequest(MessageType::Call);
  connectUpstream();
  EXPECT_CALL(upstream_connection_, close(Network::ConnectionCloseType::NoFlush));
...
TEST_F(SipRouterTest, ProtocolUpgrade) {
  initializeRouter();
  startRequest(MessageType::Call);
  ...
  MockSipObject* upgrade_response = new NiceMock<MockSipObject>();

  EXPECT_CALL(*protocol_, attemptUpgrade(_, _, _))
      .WillOnce(
          Invoke([&](Transport&, SipConnectionState&, Buffer::Instance& buffer) -> SipObjectPtr {
            buffer.add("upgrade request");
            return SipObjectPtr{upgrade_response};
          }));
  EXPECT_CALL(upstream_connection_, write(_, false))
  ...
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     ...
                     .value());
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_resp_reply")
                     .value());
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_resp_success")
                     .value());
}

// Test the case where an upgrade will occur, but the conn pool
// returns immediately with a valid, but never, used connection.
TEST_F(SipRouterTest, ProtocolUpgradeOnExistingUnusedConnection) {
  initializeRouter();
  ...
  EXPECT_CALL(context_.cluster_manager_.thread_local_cluster_.tcp_conn_pool_, newConnection(_))
      .WillOnce(
          Invoke([&](Tcp::ConnectionPool::Callbacks& cb) -> Tcp::ConnectionPool::Cancellable* {
            ...
            EXPECT_CALL(*protocol_, attemptUpgrade(_, _, _))
                .WillOnce(Invoke(
                    [&](Transport&, SipConnectionState&, Buffer::Instance& buffer) -> SipObjectPtr {
                      buffer.add("upgrade request");
                      return SipObjectPtr{upgrade_response};
                    }));
...
TEST_F(SipRouterTest, ProtocolUpgradeSkippedOnExistingConnection) {
  initializeRouter();
  startRequest(MessageType::Call);
  ...
  EXPECT_CALL(*protocol_, attemptUpgrade(_, _, _))
      .WillOnce(Invoke([&](Transport&, SipConnectionState&, Buffer::Instance&) -> SipObjectPtr {
        return nullptr;
      }));
...
TEST_F(SipRouterTest, PoolTimeoutUpstreamTimeMeasurement) {
  initializeRouter();
  ...
  EXPECT_CALL(cluster_scope, counter("sip.upstream_rq_call"));

  startRequest(MessageType::Call);

  dispatcher_.time_system_.advanceTimeWait(std::chrono::milliseconds(500));
  EXPECT_CALL(cluster_scope,
              histogram("sip.upstream_rq_time", Stats::Histogram::Unit::Milliseconds))
      .Times(0);
  EXPECT_CALL(
      cluster_scope,
      deliverHistogramToSinks(testing::Property(&Stats::Metric::name, "sip.upstream_rq_time"), 500))
      .Times(0);
  EXPECT_CALL(callbacks_, sendLocalReply(_, _))
      .WillOnce(Invoke([&](const DirectResponse& response, bool end_stream) -> void {
        auto& app_ex = dynamic_cast<const AppException&>(response);
...
TEST_P(SipRouterFieldTypeTest, OneWay) {
...
TEST_P(SipRouterFieldTypeTest, Call) {
  FieldType field_type = GetParam();
...
TEST_P(SipRouterFieldTypeTest, CallWithUpstreamRqTime) {
  FieldType field_type = GetParam();
  ...
  ON_CALL(*context_.cluster_manager_.thread_local_cluster_.cluster_.info_, statsScope())
      .WillByDefault(ReturnRef(cluster_scope));
  EXPECT_CALL(cluster_scope, counter("sip.upstream_rq_call"));
  EXPECT_CALL(cluster_scope, counter("sip.upstream_resp_reply"));
  EXPECT_CALL(cluster_scope, counter("sip.upstream_resp_success"));
  ...
  EXPECT_CALL(cluster_scope,
              deliverHistogramToSinks(
                  testing::Property(&Stats::Metric::name, "sip.upstream_rq_time"), 500));
  returnResponse();
  destroyRouter();
}

TEST_P(SipRouterFieldTypeTest, Call_Error) {
  FieldType field_type = GetParam();
  ...
  EXPECT_EQ(0UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     ...
                     .value());
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_resp_error")
                     .value());
}

TEST_P(SipRouterFieldTypeTest, Exception) {
  FieldType field_type = GetParam();
  ...
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     ...
                     .value());
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_resp_exception")
                     .value());
}

TEST_P(SipRouterFieldTypeTest, UnknownMessageTypes) {
  FieldType field_type = GetParam();
  ...
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_rq_invalid_type")
                     .value());
  EXPECT_EQ(1UL, context_.cluster_manager_.thread_local_cluster_.cluster_.info_->statsScope()
                     .counterFromString("sip.upstream_resp_invalid_type")
                     .value());
}

// Ensure the service name gets stripped when strip_service_name = true.
TEST_P(SipRouterFieldTypeTest, StripServiceNameEnabled) {
  FieldType field_type = GetParam();
...
TEST_P(SipRouterFieldTypeTest, StripServiceNameDisabled) {
  FieldType field_type = GetParam();
...
TEST_F(SipRouterTest, CallWithExistingConnection) {
  ...
  conn_state_ = std::make_unique<SipConnectionState>(3);
...
TEST_P(SipRouterContainerTest, DecoderFilterCallbacks) {
  FieldType field_type = GetParam();
  int16_t field_id = 1;
...
TEST_P(SipRouterPassthroughTest, PassthroughEnable) {
  ...
  envoy::extensions::filters::network::sip_proxy::v3::SipProtocolOptions configuration;
