...
#include "source/extensions/filters/network/sip_proxy/buffer_helper.h"

#include "test/extensions/filters/network/sip_proxy/integration.h"
...
public:
  ...
  void tryInitializePassthrough() {
    ...
    if (payload_passthrough_) {
      config_helper_
          .addFilterConfigModifier<envoy::extensions::filters::network::sip_proxy::v3::SipProxy>(
              "sip", [](Protobuf::Message& filter) {
                auto& conn_manager =
  ...
  void initializeCommon() {
    ...
    BaseSipIntegrationTest::initialize();
  }
...
INSTANTIATE_TEST_SUITE_P(TransportAndProtocol, SipConnManagerIntegrationTest,
...
TEST_P(SipConnManagerIntegrationTest, Success) {
  ...
  Stats::CounterSharedPtr counter = test_server_->counter("sip.sip_stats.request_call");
  EXPECT_EQ(1U, counter->value());
  counter = test_server_->counter("sip.sip_stats.response_success");
  EXPECT_EQ(1U, counter->value());
}

TEST_P(SipConnManagerIntegrationTest, IDLException) {
  ...
  Stats::CounterSharedPtr counter = test_server_->counter("sip.sip_stats.request_call");
  EXPECT_EQ(1U, counter->value());
  counter = test_server_->counter("sip.sip_stats.response_error");
...
TEST_P(SipConnManagerIntegrationTest, Exception) {
  ...
  Stats::CounterSharedPtr counter = test_server_->counter("sip.sip_stats.request_call");
  EXPECT_EQ(1U, counter->value());
  counter = test_server_->counter("sip.sip_stats.response_exception");
  EXPECT_EQ(1U, counter->value());
}

TEST_P(SipConnManagerIntegrationTest, EarlyClose) {
  initializeCall(DriverMode::Success);
  ...
  test_server_->waitForCounterGe("sip.sip_stats.cx_destroy_remote_with_active_rq", 1);

  Stats::CounterSharedPtr counter =
      test_server_->counter("sip.sip_stats.cx_destroy_remote_with_active_rq");
  EXPECT_EQ(1U, counter->value());
}

// Tests when the downstream client closes before completing a request but an upstream has already
// been connected/assigned.
TEST_P(SipConnManagerIntegrationTest, EarlyCloseWithUpstream) {
  initializeCall(DriverMode::Success);
...
TEST_P(SipConnManagerIntegrationTest, EarlyUpstreamClose) {
  initializeCall(DriverMode::Success);
...
TEST_P(SipConnManagerIntegrationTest, Oneway) {
  initializeOneway();
  ...
  Stats::CounterSharedPtr counter = test_server_->counter("sip.sip_stats.request_oneway");
  EXPECT_EQ(1U, counter->value());
}

TEST_P(SipConnManagerIntegrationTest, OnewayEarlyClose) {
  initializeOneway();
...
TEST_P(SipConnManagerIntegrationTest, OnewayEarlyClosePartialRequest) {
  initializeOneway();
...
class SipTwitterConnManagerIntegrationTest : public SipConnManagerIntegrationTest {};

INSTANTIATE_TEST_SUITE_P(FramedTwitter, SipTwitterConnManagerIntegrationTest,
...
TEST_P(SipTwitterConnManagerIntegrationTest, Success) {
// This test relies on an old Apache Sip Python package
